...
export default function NewCarPage() {
  ...
  const fetchCars = useCallback(async () => {
    ...
    try {
      ...
      setFetchError('Không thể tải danh sách xe mới.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchBrands = useCallback(async () => {
    try {
      ...
      // brands load failure is non-critical; form will show empty dropdown
    }
  }, []);
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <h2 style={styles.title}>Quản lý xe mới</h2>
        <button style={styles.addBtn} onClick={handleOpenAdd}>+ Thêm xe mới</button>
      </div>
      ...
      {loading ? (
        ...
        <p style={styles.emptyText}>Chưa có xe mới nào. Hãy thêm xe đầu tiên!</p>
      ) : (
